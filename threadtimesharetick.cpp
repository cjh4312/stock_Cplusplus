...
void ThreadTimeShareTick::getBuySellTimeShareTick()
{
    ...
    if (not GlobalVar::timeOutFlag[7])
    {
        initBuySellList();
...
//        if (GlobalVar::baseInfoData[0]!=0 || GlobalVar::baseInfoData[4]!=0)
    ...
    if (not GlobalVar::timeOutFlag[6])
    {
...
void ThreadTimeShareTick::initBuySellList()
{
    ...
    {
        GlobalVar::getEastData(naManager,allData,0.5,QUrl("https://datacenter-web.eastmoney.com/api/data/v1/get?sortColumns=REPORTDATE&sortTypes=-1&pageSize=1&pageNumber=1&columns=ALL&filter=(SECURITY_CODE%3D%22"+GlobalVar::curCode+"%22)&reportName=RPT_LICO_FN_CPD"),"");
        QJsonParseError jsonError;
        QJsonDocument doc = QJsonDocument::fromJson(allData, &jsonError);
        if (jsonError.error == QJsonParseError::NoError)
        {
